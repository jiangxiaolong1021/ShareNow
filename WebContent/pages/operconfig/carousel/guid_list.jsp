  ...
  <body>
    ...
    <div class="ch-container">
        ...
        <ul class="breadcrumb">
            ...
            <li>
                <a href="pages/operconfig/carousel/guid_list.jsp">引导图管理</a>
            </li>
        </ul>
    </div>
     <c:if test="${msg!=null}">
    ...
    <div class="box-header well" data-original-title="" >
        <h2><i class="glyphicon glyphicon-inbox"></i>设置手机开机引导图
        </h2>
    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                	<jsp:include page="guid_form.jsp"></jsp:include>
                </div>
			...
			"columns": [
			            ...
			            { "data": "path",
			               "render" : function(data,type, row, meta) {
			            	   return '<img height="40" width="80" src="http://localhost:8080/'+data+'"/>';
							}},
			            {  "data": "ischoose",
			               "width" : "80px",
						   "render" : function(data,type, row, meta) {
								return data=='t'?'<button type="button" class="btn btn-success" onclick="updateStae('+row.id+',\'f\')">已选</button>':'<button type="button" class="btn btn-info" onclick="updateStae('+row.id+',\'t\')">未选</button>';
							}
			            },
			            { "data": "weight" },
			            { "data": "createtime",
			  ...
			  "columnDefs": [
			            ...
			            },
			            {
			            	"targets": [7],
			            	"render" : function(data,type,row, meta) {
			            		var str='<button type="button" class="btn btn-small btn-primary btn-edit" onclick="editCarouse(\''+row.title+'\','+row.weight+',\''+row.ischoose+'\',\''+row.path+'\','+row.id+')"><i class="glyphicon glyphicon-pencil"></i>修改</button>';
			            		str+='<button class="btn btn-small btn-primary btn-edit" onclick="delCarouse('+row.id+',\''+row.ischoose+'\')"><i class="glyphicon glyphicon-trash icon-white"></i>删除</button>';
								return str;
							}
			            }
			        ],
